...
will break into different *cases* depending on what the list of points happens
...
these expressions easier to read. The idea is that if `expressionToExamine`
...
task of creating a polygon out of a list of points.  Our approach is draw a
`line` between each pair of points, as well as the first and last ones:

```
(def [c w] ['black' 10]) ; color and width

(defrec ourPolygon_ (\(points [xFirst yFirst])
  (case points
    ([[xLast yLast]]
      [ (line c w xFirst yFirst xLast yLast) ])
    ([[x1 y1] [x2 y2] | rest]
      (let lines (ourPolygon_ [[x2 y2] | rest] [xFirst yFirst])
      [ (line c w x1 y1 x2 y2) | lines ])))))
...
instead.  This exercise is completely optional for the purposes of this
tutorial.
...
it to be. In fact, let's have you put this into practice! Go ahead and complete
the following tasks:
