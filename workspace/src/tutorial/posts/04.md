...
matches one of the patterns, then the expression that is associated with that
pattern is returned. The patterns are checked from the top down, and an
underscore `_` matches anything. Think of this like the if-expressions found in
most programming languages, generalized to work on more than just Boolean values
and with more than just two different branches.
...
**Thought Exercise:** Can you guess what the Little expressions `[e1 ... en]`
and `[e1 ... en | erest]` mean?
...
A very common pattern is to use `case` in recursive functions that operate
on lists, by defining a base case for the empty list and a case for non-empty
lists. Remember the built-in `concat` function that takes a list of lists and
concatenates them into a single list? Here's how we can define it ourselves:
...
returns something based on which of three patterns it matches: if `listOfLists`
is empty, then their concatenation is the empty list; if the first list in
`listOfLists` is empty, then the concatenation is defined by recursively
concatenating the `rest` of the lists in `listOfLists`; and if the first list
has some head element `x` and tail `xs`, then the concatenation includes `x`
plus everything in `xs` and `rest`. If `ourConcat` is called with anything
besides a list, then this function will crash at run-time (by design).  If you
have functional programming experience, this will likely look like a standard
recursive definition (albeit with a different syntax than you're used to).
...
Knowing how to write functions that operate on lists, we can now return to the
task of creating a polygon out of a list of points.  Our approach is to draw a
`line` between each pair of points, as well as the first and last ones:
...
**Practice:** Try this out in Sketch-n-Sketch to draw some polygons.
...
**Exercise:** Here's an excuse to get more practice with this syntax. As it is,
calling `ourPolygon` with the list of points `[[10 10] [50 50]]` draws two
nearly-identical line segments. Add an additional case in `ourPolygon_` to make
it so that only one line segment is drawn.

**Optional Exercise:** If you know what "fold" or "reduce" means and if you're
...
Here, `fillColor` represents the color of the interior of the path (`'none'` is a valid
entry if you would like no fill); `lineColor` represents the
color of the path itself (again, `'none'` is a valid entry if you would like
...
with their control points that define the SVG path.
...
are enough to make some complicated shapes and forms.
...
Go ahead and enter this into a document in Sketch-n-Sketch, and turn on the
basic zones. See the points that are off of the line? Try manipulating them.
They are the control points for the first and second quadratic curve,
respectively. Sketch-n-Sketch supports and displays control for the other sorts
of curves in similar fashion.

**Exercise:** Now that you've seen how basic paths work, it's your turn. Try
experimenting with mixing `'L'` and curve commands. For example, you may try
drawing some of your ABCs.
...
Because SVG paths can become quite complex, we think that working with them by
mixing programmatic and direct manipulation is one area where Sketch-n-Sketch
really shines. Our built-in examples include many involved designs that rely
heavily on paths. Next, we identify two design patterns that we have found
useful.
...
**Practice:** Enter this into your document and manipulate the control points as
...
**Exercise:** Go ahead and define your own path in terms of a parameter that is
shared between all or some of the points. Try to use at least one curve and one
straight line in your design.
...
**Exercise:** Your mission, if you choose to accept, is the following:
