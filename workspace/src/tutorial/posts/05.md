...
Tho allow panning and of a fixed size window, we need only incorporate a slider
or sliders that allows the manipulation of both the x-offset and y-offset for
the window. Starting with that goal in mind, we can use a xySlider as in the
built-in examples to specify the offsets. As the upper left corner of the
editing window is the only section of the editing space that will definitely
always be in view, let's position the slider there. Then, once we've specified
the slider, we can use the values that it provides to control our currently
fixed-size viewbox.

We can specify the slider as follows (just as in the built-in example)
