...
To make this change, define two variables at the start of your program `x0` and
...
change using some simple heuristics (described in our [technical
...
Try directly manipulating various zones of the different rectangles and see what
...
**Exercise:** Now is a good time to pause and experiment a bit. Try modifying
...
the particular constants to declare you intent.
...
pattern, it is often good idea to freeze all of these indices.
...
**Exercise:** Take a look at the built-in Logo example, which is our logo for
...
TODO mention that group box can even be defined after the fact. what's nice
is that, unlike when using direct manipulation tools, it's easy to go back later
and introduce more structure if needed.
...
TODO use frozen by default instead
...
**Exercise:** Now that you have some experience with computed attributes and the
group box pattern, it's your turn to run wild! Pick the flag of a country or
city (not one that is already in the examples) and recreate it. Be sure to use a
Group Box to be able to adjust the overall size.
