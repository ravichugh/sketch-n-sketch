...
Let's define initial width, height, x-, and y-position values that will be used
...
**Optional Exercise:** Refactor the `stripes` definition with additional
variables (using `let`-expressions) to eliminate redundant calculations.

Next, we need to define the six-pointed stars that run horizontally across the
