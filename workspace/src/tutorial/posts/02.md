...
```
(def [x0 y0 w h deltaP deltaS] [100 100 450 300 10! 20!])
(def groupBox (rect [255 255 255 0] (- x0 deltaP) (- y0 deltaP) (+ w deltaS) (+ h deltaS)))
```

Where `deltaP` and `deltaS` represent the difference between the positioning
(both `x0` and `y0`) and the sizing (both `w` and `h`) of the groupBox and its
...
in the built-in examples 'US-13 Flag' or 'US-50 Flag'.
...
to use a group box to be able to adjust the overall size and position. Since we haven't
talked about polygons or curved lines yet, you'll probably want to choose a flag
that consists mostly of simple shapes for now.
...
Continue on to [Step 3][step3]. TODO: Link
...
[step3]: http://ravichugh.github.io/sketch-n-sketch/tutorial/03.html
