...
some attributes that you simply cannot manipulate directly. The simplest example
of this is color. Because of this, we have built in a zone to enable
manipulation of the color. Go ahead and change to the three boxes example or your `n`
shapes example from last lesson. Then, cycle through the Zone types until the
Color option is displayed. Next to each shape, you should see a slider that
allows you to cycle through a subset of the allowed RGB values as well as a
grayscale. When you do this, you may notice that the color field of the shape
that you are interacting with changed from what it was before to a number
between `0` and `500`. This is a non-standard representation of color that we
call a Color Number and is a product of the underlying mechanics of how we
perform the program updates. The intended use is for this widget to allow quick
sampling of different colors while the final palette will be specified with the
more full-fledged RGBA or CSS string specification, which includes Hex.

Beyond color, there are many other indirect attributes that manifest themselves
readily on larger projects. In case you weren't convinced in the last lesson, we're 
going to go through an example now where there is a parameter
of the design that absolutey cannot be manipulated directly in the design. Then, we're going
to demonstrate how it is simple to make tools to allow for their manipulation
anyways.
...
slider. The argument `roundInt` is a boolean (`true` or `false`) that indicates
whether or not the variable that it modifies should be rounded to the nearest
integer. `xStart` and `xEnd` indicate the x-positions on the canvas that the
slider should start and stop at, while `y` indicates the y-position. `minVal`
and `maxVal` are the minimum and maximum respective values that the slider
covers for the variable that it specifies, and `caption` provides a label.
Finally, `curVal` represents the current value of the variable that it
determines, and it is set when first creating the slider to indicate the initial
value. 
Usually this is paired with a `def` or `let` statement in the following
way:
...
Then, `val` can be used in your program like any other variable. Remember, in
order for the slider to appear it must be appended to the `svg` statement at the
end of your program. To specify a slider that accomplishes the same thing as our
helper box, we could enter the following in place of the `helperShape`
definition:

```
(def [totalNumber numSlider] (hSlider true 100! 350! 1! 7! "Number of Boxes: " 3))
; Code using totalNumber
(svg (concat [ boxes numSlider ]))
```

Go ahead and change your example to use a slider instead of a helper box, run
the program, and then adjust the number of boxes using it.

For more inspiration, we've provided other sample slider implementations in the
built-in examples under Widgets and 2D Slider. Coming up with useful widgets and
reusing them in later projects is a good idea, and feel free to copy what we've
done if it helps your workflow.
...
big example is worth what we might get from it in this portion.
P4(?)
...
to tackle a bigger project! Go ahead and do the following things (in order would
probably be easiest):

* Choose a design that you would like to create. It should involve at least
    three kinds of shapes and be complicated enough to be interesting to you.
* Choose a subset of the design that is all one shape and such that the elements
    are logically linked to one another. Write a placement function and map it
    over a list of indices or a list of some parameter to generate that subset
    of the image.
* If possile, reduce that list of indices or parameters to an expression that is
    determined according to only one or two variables. `range` is very handy for
    simplifying index lists and mapping a `generateParameter` function over a
    list of indices generated by `range` is also a good way to generate a list
    of logically linked parameters.
* Create or use an existing UI Widget to manipulate this parameter or
    parameters.
* Pick another subset of this design and repeat the above process. Continue this
    until you've completed the design, feeling free to 'turn off' the UI Widgets
    that you are not using anymore as well as freezing parameters that you have
    set how you like.

To get the creative juices flowing, here are some suggested project ideas, in no
particular order:
...
* A minimalist art piece with repetitive elements. Feel free to channel your
    inner
