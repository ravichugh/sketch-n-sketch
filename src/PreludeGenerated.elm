...
(typ circleLeftTop (-> Circle Point))
(def circleLeftTop (\\circle
  (let [cx cy] (circleCenter circle)
    [(- cx (circleRadius circle)) (- cy (circleRadius circle))]
  )
))

(typ circleCenterTop (-> Circle Point))
(def circleCenterTop (\\circle
  (let [cx cy] (circleCenter circle)
    [cx (- cy (circleRadius circle))]
  )
))

(typ circleRightTop (-> Circle Point))
(def circleRightTop (\\circle
  (let [cx cy] (circleCenter circle)
    [(+ cx (circleRadius circle)) (- cy (circleRadius circle))]
  )
))

(typ circleRightCenter (-> Circle Point))
(def circleRightCenter (\\circle
  (let [cx cy] (circleCenter circle)
    [(+ cx (circleRadius circle)) cy]
  )
))

(typ circleRightBot (-> Circle Point))
(def circleRightBot (\\circle
  (let [cx cy] (circleCenter circle)
    [(+ cx (circleRadius circle)) (+ cy (circleRadius circle))]
  )
))

(typ circleCenterBot (-> Circle Point))
(def circleCenterBot (\\circle
  (let [cx cy] (circleCenter circle)
    [cx (+ cy (circleRadius circle))]
  )
))

(typ circleLeftBot (-> Circle Point))
(def circleLeftBot (\\circle
  (let [cx cy] (circleCenter circle)
    [(- cx (circleRadius circle)) (+ cy (circleRadius circle))]
  )
))

(typ circleLeftCenter (-> Circle Point))
(def circleLeftCenter (\\circle
  (let [cx cy] (circleCenter circle)
    [(- cx (circleRadius circle)) cy]
  )
))
...
(typ ellipseLeftTop (-> Ellipse Point))
(def ellipseLeftTop (\\ellipse
  (let [cx cy] (ellipseCenter ellipse)
    [(- cx (ellipseRadiusX ellipse)) (- cy (ellipseRadiusY ellipse))]
  )
))

(typ ellipseCenterTop (-> Ellipse Point))
(def ellipseCenterTop (\\ellipse
  (let [cx cy] (ellipseCenter ellipse)
    [cx (- cy (ellipseRadiusY ellipse))]
  )
))

(typ ellipseRightTop (-> Ellipse Point))
(def ellipseRightTop (\\ellipse
  (let [cx cy] (ellipseCenter ellipse)
    [(+ cx (ellipseRadiusX ellipse)) (- cy (ellipseRadiusY ellipse))]
  )
))

(typ ellipseRightCenter (-> Ellipse Point))
(def ellipseRightCenter (\\ellipse
  (let [cx cy] (ellipseCenter ellipse)
    [(+ cx (ellipseRadiusX ellipse)) cy]
  )
))

(typ ellipseRightBot (-> Ellipse Point))
(def ellipseRightBot (\\ellipse
  (let [cx cy] (ellipseCenter ellipse)
    [(+ cx (ellipseRadiusX ellipse)) (+ cy (ellipseRadiusY ellipse))]
  )
))

(typ ellipseCenterBot (-> Ellipse Point))
(def ellipseCenterBot (\\ellipse
  (let [cx cy] (ellipseCenter ellipse)
    [cx (+ cy (ellipseRadiusY ellipse))]
  )
))

(typ ellipseLeftBot (-> Ellipse Point))
(def ellipseLeftBot (\\ellipse
  (let [cx cy] (ellipseCenter ellipse)
    [(- cx (ellipseRadiusX ellipse)) (+ cy (ellipseRadiusY ellipse))]
  )
))

(typ ellipseLeftCenter (-> Ellipse Point))
(def ellipseLeftCenter (\\ellipse
  (let [cx cy] (ellipseCenter ellipse)
    [(- cx (ellipseRadiusX ellipse)) cy]
  )
))
