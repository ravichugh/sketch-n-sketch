...
function clearCSSClasses() {
    document.getElementById("autogenerated_css_classes").innerHTML = "";
}

function makeCSSClass(colorStr) {
    autogenName = ".autogenerated_class_" + classNum.toString(16);
    classNum++;
...
runtime.ports.aceInTheHole.subscribe(function(codeBoxInfo) {
    ...
    for (hi in codeBoxInfo.highlights) {
        ...
        var aceRange = new Range(hiRange.start.row,
                                 hiRange.start.column,
                                 hiRange.end.row,
                                 hiRange.end.column);
        var hiClass = makeCSSClass(codeBoxInfo.highlights[hi].color);
        editor.getSession().addMarker(aceRange,hiClass,false);
    }
