...
(def main
  (draw []))
; <!-- PROSE --> <p>The structure of a program is a series of top-level definitions. The last definition is called main and describes the image to draw in the canvas area on the right. The initial main expression has an empty list, so nothing is drawn.</p><h3 id=\"exercises\">Exercises</h3><ol style=\"list-style-type: decimal\"><li><p>Add the expression <code>(show &quot;hello world!&quot;)</code> to the list of drawings. Notice how the border of the canvas turns red; this indicates that the code needs to be re-run. Hit the Run button.</p></li><li><p>Now define this expression in a new variable instead, and refer to it in the main expression.</p></li><li><p>Add extra parentheses somewhere and re-run. You will get a syntax error. Many expressions start and end with parentheses; extra parentheses are not allowed.</p></li></ol>
...
(def main
  (draw [blueSquare]))
; <!-- PROSE --> <p>Here is a red square. Unfortunately, the name of the function that generates this red square is incorrect. To fix this, we can use the <em>Rename</em> tool to rename the function at its definition and its call-sites.</p><p>There are three ways to do this:</p><ul class=\"_123\"><li class=\"_1\"><h3>Text-Select Mode (right-click menu)</h3><p>Select the text <code>blueSquare</code>, right-click the editor, and select the <em>Rename</em> tool from the pop-up menu. Then, enter a new name (like <code>redSquare</code>) in the results list and press Enter.</p></li><li class=\"_2\"><h3>Text-Select Mode (Code Tools menu)</h3><p>Select the text <code>blueSquare</code>, and select the <em>Rename</em> Variable tool from the Code Tools menu at the top left of the screen. Then enter a new name (like <code>redSquare</code>) in the results list and press Enter.</p></li><li class=\"_3\"><h3>Box-Select Mode</h3><p>Hold down the Shift key, hover over <code>blueSquare</code>, click the box that pops up over <code>blueSquare</code>, and then release the Shift key. Select the <em>Rename</em> tool from the pop-up menu. Enter a new name (like <code>redSquare</code>) in the results list and press Enter.</p></li></ul><h3 id=\"exercises\">Exercises</h3><ol style=\"list-style-type: decimal\"><li>Perform the renaming with each of the three interactions. Use Undo after each to reset the program.</li></ol>
...
(def main
  (draw [redSquare]))
; <!-- PROSE --> <p>Many times, when using a function like <code>rect</code>, passing in multiple constants rather than named variables can lead to confusion as to the purpose of each constant.</p><p>To alleviate this, we can introduce variables using the <em>Introduce Variable</em> tool. This tool uses the notion of a <em>target position</em>, which is the whitespace between or above other items in the code. Target positions typically allow us to select where we want resultant transformations to appear in the code. In the case of the <em>Introduce Variable</em> tool, the target position indicates where we would like the variables to be introduced.</p><p>Let's try out the <em>Introduce Variable</em> tool on the x- and y- positions of the red square (the first two arguments of the <code>rect</code> function after the color, &quot;salmon&quot;).</p><p>As before, we can use this tool in three ways:</p><ul class=\"_123\"><li class=\"_1\"><h3>Text-Select Mode (right-click menu)</h3><p>Text-select one of the <code>100</code>s, right-click, then select <em>Introduce Variable</em> from the pop-up menu. The configuration panel asks for one or more expressions for which variables will be introduced, as well as an optional target position. Click the other <code>100</code>, then click a target position before the call to rect or above the def. Apply the transformation from the results list.</p></li><li class=\"_2\"><h3>Text-Select Mode (Code Tools menu)</h3><p>Text-select one of the <code>100</code>s, then select <em>Introduce Variable</em> from the Code Tools menu. The configuration panel asks for one or more expressions for which variables will be introduced, as well as an optional target position. Click the other <code>100</code>, then click a target position before the call to <code>rect</code> or above the def. Apply the transformation from the results list.</p></li><li class=\"_3\"><h3>Box-Select Mode</h3><p>Hold down Shift, then click on both occurrences of the number <code>100</code> as well as a target position before the call to <code>rect</code> or above the def. Release the Shift key and select <em>Introduce Variables</em> from the pop-up menu. Apply the transformation from the results list.</p></li></ul><h3 id=\"exercises\">Exercises</h3><ol style=\"list-style-type: decimal\"><li>Introduce x and y variables with each of the three interactions. Use Undo after each to reset the program.</li></ol>
...
(def main
  (draw [redSquare]))
; <!-- PROSE --> <p>The <em>Introduce Variable</em> tool can be used to give names to multiple expressions at once, but sometimes we may want a single variable to take the place of multiple expressions. The <em>Introduce Single Variable</em> tool does just that.</p><p>In this code snippet, the rectangle should have sides of equal length, but the width and height (fourth and fifth) arguments of the <code>rect</code> function call are not equal.</p><p>As with all Sketch-n-Sketch code tools, there are three ways that we can use the <em>Introduce Single Variable</em> tool:</p><ul class=\"_123\"><li class=\"_1\"><h3>Text-Select Mode (right-click menu)</h3><p>Text-select either <code>120</code> or <code>80</code>. Then, right-click and select <em>Introduce Single Variable</em>. Click on the remaining number (either <code>120</code> or <code>80</code>). Click on one of the results from the list that appears to apply the transformation.</p><p><strong>Note</strong>: in Text-Select Mode, a SINGLE, primary argument is selected FIRST, then the desired tool is selected, and then the rest of the arguments are selected AFTERWARD (as required by the configuration panel).</p></li><li class=\"_2\"><h3>Text-Select Mode (Code Tools menu)</h3><p>Text-select either <code>120</code> or <code>80</code>. Then, click on the Code Tools menu at the top of the screen and select <em>Introduce Single Variable</em>. Click on the remaining number (either <code>120</code> or <code>80</code>). Click on one of the results from the list that appears to apply the transformation.</p><p><strong>Note</strong>: in Text-Select Mode, a SINGLE, primary argument is selected FIRST, then the desired tool is selected, and then the rest of the arguments are selected AFTERWARD (as required by the configuration panel).</p></li><li class=\"_3\"><h3>Box-Select Mode</h3><p>Hold down Shift, then click on <code>120</code> and <code>80</code> Release the Shift key and select <em>Introduce Single Variable</em> from the pop-up menu. Click on one of the results from the list that appears to apply the transformation.</p><p><strong>Note</strong>: in Box-Select Mode, ALL arguments are selected first, BEFORE the desired code transformation is selected.</p></li></ul><h3 id=\"exercises\">Exercises</h3><ol style=\"list-style-type: decimal\"><li>Fix the <code>redSquare</code> definition using each of the three interactions. Use Undo after each interaction to reset the program.</li></ol>
