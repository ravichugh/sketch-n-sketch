    ...
    helper acc xs seed =
      case xs of
        ...
        head :: tail ->
          let
            (newAcc, newSeed) =
              step (f head acc) seed
          in
            helper newAcc tail newSeed
  in
    Generator (helper acc xs)

sequence : List (Generator a) -> Generator (List a)
sequence =
  fold (\xGen xs -> map (\x -> x :: xs) xGen) []
    >> map List.reverse

sampleUnique : List (Int, Generator a) -> Generator (Set a)
sampleUnique =
  let
    helper (total, gen) acc seed =
      if Set.size acc >= total then
        ...
        let
          (x, newSeed) =
            step gen seed
        in
          helper (total, gen) (Set.insert x acc) newSeed
  in
    fold
