...
addShapeToModel model newShapeName newShapeExp =
  let newProgram =
    DrawAddShape.addShape model (always True) (Just newShapeName) newShapeExp (Just 1) Nothing Nothing Nothing False model.inputExp
    |> List.head
    |> Maybe.withDefault model.inputExp
  in
