...
desugarExp langExp =
  case Lang.unwrapExp langExp of
    Lang.EConst _ num loc wd             -> EString "TinyStructuredEditorsForLowLowPrices core language does not support numeric constant literals"
    ...
    Lang.ERecord _ mw decls _                                 -> EString "TinyStructuredEditorsForLowLowPrices core language does not yet support records"
    ...
    Lang.ECase _ scrutineeExp branches _ ->
      let
        patToMaybeCtorNameAndArgPats pat =
          case pat.val.p__ of
            Lang.PRecord _ entries _ ->
              let
                namePatPairs = Utils.recordKeyValuePairs entries

                maybeCtorName =
                  Utils.maybeFind Lang.ctorDataType namePatPairs
                  |> Maybe.andThen LeoUnparser.patToMaybeCtorName

                maybeCtorArgPats =
                  Utils.maybeFind Lang.ctorArgs namePatPairs
                  |> Maybe.andThen LeoUnparser.patToMaybeCtorArgEntries
                  |> Maybe.map LeoUnparser.ensureRecordNumericArgEntriesInOrder
                  |> Maybe.map Utils.recordValues
              in
              Maybe.map2 (,) maybeCtorName maybeCtorArgPats
        ...
        maybeDesugaredBranches =
          ...
          |> List.map
              (\(pat, branchExp) ->
                case patToMaybeCtorNameAndArgPats pat of
  ...
  case cache |> Utils.findFirst (\(cachedLangVal, cachedTaggedVal) -> Javascript.tripleEqualsOperator langVal cachedLangVal) of
    ...
    Nothing ->
      ...
      case langVal.v_ of
        ...
        Lang.VRecord d                         -> ret cache <| vString "TinyStructuredEditorsForLowLowPrices core language does not yet support records"
        Lang.VConst offsetProvenance (num, tr) -> ret cache <| vString <| toString num
        ...
        -- Lang.VClosure [singleRecName] pats bodyExp funcEnv  ->
        --   -- Binarize all functions.
        --   case pats |> List.map LangTools.patToMaybePVarIdent |> Utils.projJusts of
        --     Just (firstArgName::otherArgNames) ->
        --       -- Handle otherArgNames by wrapping bodyExp with a bunch of one-arg functions.
        ...
        -- Lang.VClosure recNames _ _ funcEnv -> ret cache <| vString <| "TinyStructuredEditorsForLowLowPrices core language does not yet support (mutually) recursive functions (recnames: " ++ Utils.toSentence recNames ++ ") (env names: " ++ Utils.toSentence (List.map Tuple.first funcEnv) ++ ")"
