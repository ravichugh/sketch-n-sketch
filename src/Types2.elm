...
inferType gamma stuff thisExp =
  case (unExpr thisExp).val.e__ of
    ...
    EVar ws x ->
      -- We already set the type of this variable because it is special (like a
      -- tuple get).
      if (unExpr thisExp).val.typ /= Nothing then
        { newExp = thisExp
        , holeEnv = emptyHoleEnv
        }
      else
      ...
      in
        case Lang.toTupleGet thisExp of
          Just (n, i, arg) ->
            let
              argResult =
                inferType gamma stuff arg
            in
              case (unExpr argResult.newExp).val.typ of
                Just argType ->
                  case unwrapType argType of
                    TRecord _ _ decs _ ->
                      case
                        decs
                          |> tupleEncodingUnapply
                          |> Maybe.map (List.map Tuple.second)
                      of
                        Just argTypeComponents ->
                          if List.length argTypeComponents == n then
                            case Utils.maybeGeti1 i argTypeComponents of
                              Just returnType ->
                                let
                                  getType =
                                    rebuildArrow ([], [argType], returnType)

                                  newFunc =
                                    setType (Just getType) eFunc
                                in
                                  { newExp =
                                      EApp ws1 newFunc [argResult.newExp] apptype ws2
                                        |> withDummyExpInfo
                                        |> setType (Just returnType)
                                  , holeEnv =
                                      argResult.holeEnv
                                  }
    ...
    (EFun ws1 pats body ws2, _, Just (typeVars, argTypes, retType)) ->
      ...
      in
        if List.length pats < List.length argTypes then
          let
            newRetType =
              rebuildArrow (typeVars, List.drop (List.length pats) argTypes, retType)

            result =
              checkType newGamma stuff body newRetType
          in
            if result.okay then
              { okay = True
              , newExp =
          ...
          , newExp =
              thisExp
                |> setDeuceTypeInfo
                     ( deucePlainLabels <|
                         "More patterns than argument types supplied"
