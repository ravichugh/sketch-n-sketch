...
<li>You'll see that the code is updated accordingly:@displaycode</li>
...
@replacepath(placeholder 3)<|"""type CutOff = CutOff String Num
...
htmlpass htmlnode = 
  let aux src htmlnode = case htmlnode of
    ...
    ["replacepath", ["path", path]::["code", code]::attrs, []] ->
      let newSrc = Regex.replace (escape path) (\_ -> code) src in
      (newSrc, <span>Replace the code <code>@path</code> (@(positionOf path src)) with<code @attrs>@code</code></span>)
