            ...
            case d of
              ...
              ListElemDelete count ->
                let newNode = Html.replace refRegex (\{submatches=[ref],match} ->
                  ...
                  else if nref >= j + 1 + offset && nref <= lenReferences then [["TEXT", """[0] (deleted ref to '@(Dict.get ref referencesDict |> Maybe.withDefault "?"))')"""]]
                  else [["TEXT", match]]
                  ) currentNode
                in
                aux (offset - count) newNode True diffsTail
...
-- We should not remove references or insert them in this list, instead, we should do these
-- operations from the output to keep in sync with the text
references = ["Mayer", "Chugh et al. 2016", "Last missing paper"]
...
<p>
  ...
  You can change any reference title by simply editing it, either in the UL, or even in the title of a citation. For example, change the first "Mayer" to "Game Programming by Demonstration, Mayer et Kuncak, 2013".
</p>
<p>
  <b>Adding or inserting references:</b>
  Position the cursor at the end of an element in the list, press ENTER and enter '[1] The reference name',
  and <code>addReferences</code> takes care of shifting indices. Try to insert "[4] Mayer, Chugh and Kuncak 2016".
</p>
<p>
  <b>Deleting references:</b>
  If you delete a reference, <code>addReferences</code> takes care of transforming any citation to [0] and inlines the name. Try to delete '[2] Work that is not that relevant really' below.
</p>
<p>
  <b>Insert, delete and modify citations</b>
  You can delete any citation that looks like [1], insert new ones by typing [2] for example, or change a citation by changing its number.
</p>
</span>
<div style="border:4px solid black;padding:20px" id="example">
@addReferences(references)<|<span>
<h2>Related work</h2>
The work in [1] both suggested that it is possible to modify an interface without modifying the code.
However, it also pin-points that neglecting raw access to the code is a key impedance to adoption.
The work of [2] (deleted ref to 'Work that is not that relevant really)') might also be relevant.
In [2], the authors demonstrate that dual editing of both code and view increase the productivity.
[3] applies a more general-purpose approach to edit programmatically generated documents such as this one.
</span></div>
</div>